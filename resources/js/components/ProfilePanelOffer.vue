<template>
	<div id="offer-panel" class="container">
		...
		<div class="panel-alert alert alert-secondary" role="alert">
			Añade y cataloga los productos que deseas ofertar. Cada producto
			puede tener 1 imagen y hasta 3 certificaciones.
			<br />
			<i
		...
		<div class="container">
			<div class="table-responsive mt-5">
				<table class="table table-hover">
					...
					<tbody>
						<tr
							v-for="data_offer in data_offers"
							:key="data_offer.offer.id"
						>
							<td>
							...
							<td>
								<button
									class="btn btn-light btn-category"
									style="border: 1px solid #cecece;"
								>
									<img
										:src="
											data_offer.offer_category[0].picture
										"
										class="img-fluid"
										style="height: 40px;"
									/>
							...
							<td>
								...
								<div class="container mb-1">
									<button
										...
										@click="
											deleteProduct(data_offer.offer.id)
										"
									>
										Eliminar
									</button>
			...
			<div class="mt-5 text-center" v-if="data_offers.length === 0">
				<p style="color: #6C757D; font-size: 25px;">
					Aún no tienes productos ofertados
				</p>
			</div>
	...
	methods: {
		...
		addProduct() {
			Swal.fire({
				...
				if (res.value == true) {
					axios
						.post("/profile/add-product", formData, config)
						.then(res => {
							console.log(res);
							Swal.fire({
		...
		deleteProduct(product_id) {
			Swal.fire({
				...
				if (res.value == true) {
					axios
						.delete(`/profile/delete-product/${product_id}`)
						.then(res => {
							console.log(res);
							Swal.fire({
