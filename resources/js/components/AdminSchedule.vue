	...
	<div>
		...
		<div v-else class="row mt-3">
			...
			>
				<div
					class="p-3 mt-2"
					style="border: 1px solid #ecec; border-radius: 6px;"
				>
					<div
					...
					>
						...
						<div class="btn-group" role="group">
							<button
								...
								@click="enableEdit(avalible_time.id)"
							>
								Editar
							</button>
							<button
								...
								@click="deleteAvalibleTime(avalible_time.id)"
							>
								Eliminar
							</button>
						</div>
					...
					<div class="row">
						<div class="form-group col-md-6 row">
							<label
								class="col-sm-4"
								:for="'inputStartTime' + avalible_time.id"
								>Desde</label
							>
							<div class="col-sm-8">
								<input
									type="time"
									step="3600"
									:id="'inputStartTime' + avalible_time.id"
									v-model="avalible_time.startTime"
									:class="
										'form-control inputItem' +
											avalible_time.id
									"
									disabled
								/>
						...
						<div class="form-group col-md-6 row">
							<label
								class="col-sm-4"
								:for="'inputEndTime' + avalible_time.id"
								>Hasta</label
							>
							<div class="col-sm-8">
								<input
									type="time"
									step="3600"
									:id="'inputEndTime' + avalible_time.id"
									v-model="avalible_time.endTime"
									:class="
										'form-control inputItem' +
											avalible_time.id
									"
					...
					<div class="container row">
						<div
							v-for="(day, index) in avalible_time.daysOfWeek"
							:key="index"
						...
						>
							<input
								type="checkbox"
								:class="
									'custom-control-input inputItem' +
										avalible_time.id
								"
								:id="'input' + avalible_time.id + 'Day' + index"
								v-model="day.value"
								disabled
							/>
							<label
								:for="
									'input' + avalible_time.id + 'Day' + index
								"
					...
					<button
						type="button"
						@click="editAvalibleTime(avalible_time.id)"
						:class="
							'btn btn-success btn-block inputItem' +
								avalible_time.id
						"
						style="display: none;"
						disabled
						...
						<div class="modal-body p-4">
							...
							<div class="container row mt-2">
								...
								>
									<input
										type="checkbox"
										class="custom-control-input inputItem"
										:id="'checkboxDay' + day.id"
										v-model="day.value"
									/>
									<label
										:for="'checkboxDay' + day.id"
										class="custom-control-label"
										>{{ day.name }}</label
									>
								</div>
...
export default {
		...
		axios
			...
			.then(res => {
				...
				this.avalible_times.forEach(event => {
					let daysOfWeek = event.daysOfWeek
						.split("")
	...
	methods: {
		enableEdit(item_id) {
			...
			for (let i = 0; i < elements.length; i++) {
				if (elements[i].disabled) {
					...
					let buttonEdit = (document.querySelector(
						`button.inputItem${item_id}`
					).style.display = "");
				} else {
		...
		recuperarDatos(item_id) {
			let startTime = document.getElementById(`inputStartTime${item_id}`)
				.value;
			let endTime = document.getElementById(`inputEndTime${item_id}`)
				.value;
			...
			for (let i = 0; i < this.days.length; i++) {
				selectedDays[i] = {
					id: i,
					value: document.getElementById(`input${item_id}Day${i}`)
						.checked
				};
			}
