<template>
	<div id="cert-panel" class="container">
		...
		<div class="panel-alert alert alert-secondary" role="alert">
			Añade las certificaciones o acreditaciones de tu empresa.
			Dependiendo de tu plan puedes añadir cierta cantidad de
			certificaciones en formato pdf.
			<br />
			<i
						...
						<tr v-for="cert in certifications" :key="cert.id">
							...
							<td>
								<a
									:href="cert.url"
									target="_blank"
									class="btn btn-light btn-cert text-center"
									style="border: 1px solid #cecece;"
								>
									<span style="margin: auto;"
										><i
											class="fas fa-file-pdf"
											style="color: rgb(213, 72, 65);"
										>
										</i>
						...
						<div class="form-group">
							...
							<div class="custom-file">
								...
								<label
									...
									{{ upload_cert_name }}</label
								>
								<small
									><span class="text-muted"
										>Solamente se aceptará archivos de
										formato pdf</span
									></small
								>
							</div>
	...
	methods: {
		...
		addCert() {
			Swal.fire({
				...
				if (res.value == true) {
					...
					axios
						.post("/profile/add-cert", formData, config)
						.then(res => {
							console.log(res);
							Swal.fire({
		...
		deleteCert(cert_id) {
			Swal.fire({
				...
				if (res.value == true) {
					axios
						.delete(`/profile/delete-cert/${cert_id}`)
						.then(res => {
