							...
							>
								...
								<div class="content">
									...
									<button
										@click="login_user()"
										type="button"
										class="btn-block btn-hover color-1"
									>
										Iniciar Sesión
									</button>
									<button
										type="button"
										class="btn-block btn-gradient-zv d-none btnLoad"
									>
										<img
											src=""
											alt=""
								...
								<div class="content">
									...
									<div class="group">
										<select
											id="signin-type"
											placeholder=" "
											v-model="register.type_id"
										>
											<!--<option class="hidden" selected disabled>Seleccion una opción</option>-->
											<option value="1"
												>Vendedor (Productor)</option
											>
											<option value="2"
												>Vendedor (Acopiador)</option
											>
									...
									<button
										type="button"
										class="btn-block btn-gradient-zv d-none btnLoadRegister"
									>
										<img
											src=""
											alt=""
...
<script>
export default {
	props: ["csrf", "href_login", "href_register", "href_reset_password"],
	data() {
		return {
			login: {
				email: "",
	...
	methods: {
		...
		register_user: function() {
			...
			if (validation.value) {
				...
				if (this.register.accept_terms) {
					...
					axios
						.post(this.href_register, {
							name: this.register.name,
							email: this.register.email,
							password: this.register.password,
							ruc: this.register.ruc,
							legal_registration: this.register
								.legal_registration,
							type_id: this.register.type_id,
							password: this.register.password,
							password_confirmation: this.register
								.password_confirmation,
						...
						.then(res => {
							Swal.fire({
								...
								text:
									"Por favor verifica tu correo electronico para validar tu cuenta",
								type: "success",
								showConfirmButton: false,
								timer: 2000, // Mensaje dura 2s
								// Quitar pantalla de carga
						...
						.catch(error => {
							if (error.response.status === 500) {
								...
								window.Swal.fire({
									title: "RUC ya usado",
									text:
										"Por favor utiliza otro RUC o comunicate con nosotros nosotros.",
									type: "error",
									timer: 2000, // Mensaje dura 2s
									// Quitar pantalla de carga
									onBeforeOpen: () => {
								...
								if (
									...
									if (
										...
										window.Swal.fire({
											...
											text:
												"Por favor utiliza otra dirección de email o comunicate con nosotros.",
											type: "error",
											timer: 2000, // Mensaje dura 2s
											// Quitar pantalla de carga
											onBeforeOpen: () => {
					...
					window.Swal.fire({
						title:
							"Debe aceptar nuestros terminos y condiciones para poder continuar",
						type: "info",
						showConfirmButton: false,
						timer: 2000, // Mensaje dura 2s
						// Quitar pantalla de carga
		...
		register_form_validation() {
			...
			if (
				...
				this.register.ruc !== "" &&
				this.register.legal_registration !== "" &&
				this.register.type_id !== "" &&
				this.register.password_confirmation !== ""
			) {
				if (
					this.register.legal_registration.length <= 20 &&
					this.register.ruc.length <= 20 &&
					this.register.password.length >= 8 &&
					this.register.password_confirmation.length >= 8
				) {
					if (
						this.register.password ===
						this.register.password_confirmation
...
#pass-check:checked + label:before,
#tc-check:checked + label:before {
	border-color: #88be2e;
	border-top: 0;
	border-left: 0;
	width: 8px;
