<template>
	<div id="meet-panel" class="container">
		...
		<div class="panel-alert alert alert-secondary" role="alert">
			Gestiona tus agendamientos. Aquí podrás ver a las empresa que estan
			interesadas en tus productos y desean contactarse contigo.
			<br />
			<i
		...
		<div class="container">
			<div class="text-center panel-info-subtitle mt-4">
				<label class="text-uppercase h5"
					>Datos para el agendamiento</label
				>
			</div>
			<div class="container">
				<div class="form-group">
					<label
						><img src="/img/iconos/hangout_logo.png" height="17" />
						Hangouts</label
					>
					<input
						type="text"
						class="form-control"
						placeholder="Perfil de Hangouts"
						v-model="hangouts_url"
					/>
				</div>
				<div class="form-group">
					<label
						><img src="/img/iconos/skype_logo.png" height="17" />
						Skype</label
					>
					<input
						type="text"
						class="form-control"
						placeholder="Perfil de Skype"
						v-model="skype_url"
					/>
				</div>
				<button
					@click="editMeetInfo()"
					class="btn btn-lg btn-block btn-save"
			...
			<div class="container">
				<a
					href="/schedule"
					class="btn btn-block btn-lg btn-save"
					style="margin-top: 25px;"
				>
					Ver calendario
				</a>
			...
			<div class="table-responsive mt-1 table-meet">
				<table class="table table-hover">
					...
					<tbody>
						<tr
							v-for="data_meeting in data.received_meetings"
							:key="data_meeting.meeting.id"
							class="text-center"
						>
							<td>
								<profile-modal-meet
							...
							<td>
								<p class="text-muted">
									...
									{{
										message_date(
											data_meeting.meeting.created_at
										)
									}}
								</p>
								<p class="meet-state">
							...
							<td>
								...
								>
									<div class="container mb-1">
										<button
											class="btn btn-danger disabled btn-block"
											disabled
										>
											Cancelado
										</button>
			...
			<div
				...
				v-if="data.received_meetings.length === 0"
			>
				<p style="color: #6C757D; font-size: 25px;">
					Aún no tienes agendamientos recibidos
				</p>
			</div>
			...
			<div class="table-responsive mt-1 table-meet">
				<table class="table table-hover">
					...
					<tbody>
						<tr
							v-for="data_meeting in data.sended_meetings"
							:key="data_meeting.id"
							class="text-center"
						>
							<td>
							...
							<td>
								...
								>
									<div class="container mb-1">
										<button
											class="btn btn-secondary disabled btn-block"
											disabled
										>
											Por confirmar
										</button>
								...
								>
									<div class="container mb-1">
										...
										<small class="text-muted"
											>Esperando la confirmacion de hora y
											fecha</small
										>
									</div>
								</div>
			...
			<div
				class="mt-5 text-center"
				v-if="data.sended_meetings.length === 0"
			>
				<p style="color: #6C757D; font-size: 25px;">
					Aún no tienes agendamientos enviados
				</p>
			</div>
		</div>
	</div>
		...
		editMeetInfo() {
			Swal.fire({
				...
				if (res.value === true) {
					...
					axios
						.post("/profile/update-contact-info", {
				...
				if (res.value) {
					...
					axios
						...
						.catch(err => {
							...
							Swal.fire({
								title:
									"Error al cargar la operación, vuelva a cargar la página",
								type: "error"
							});
						});
				}
