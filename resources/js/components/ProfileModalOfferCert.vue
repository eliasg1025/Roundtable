<template>
	<div
		:id="'eeadsas' + data_offer.offer.id"
		class="modal fade"
		tabindex="-1"
		role="dialog"
		aria-hidden="true"
			...
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">
						Certificados de: {{ data_offer.offer.title }}
					</h5>
					<button
						type="button"
						class="close"
				...
				<div class="modal-body">
					...
					<div
						...
						v-if="data_offer.certifications.length < 3"
					>
						<button
							class="btn btn-add"
							data-toggle="modal"
							:data-target="
								'#modalAddOfferCert' + data_offer.offer.id
							"
						>
						...
						<small class="mt-1">
							<span class="text-muted"
								>Solo puedes agregar 3 certificados por
								producto.</span
							>
						</small>
					</div>
					...
					<div
						...
						v-if="data_offer.certifications.length === 0"
					>
						<p style="color: #6C757D; font-size: 25px;">
							Este producto aún no tiene certificados.
						</p>
					</div>

					<!-- Modal Add OfferCert -->
					<div
						:id="'modalAddOfferCert' + data_offer.offer.id"
						class="modal fade"
						tabindex="-1"
						role="dialog"
						aria-hidden="true"
					>
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									<h5 class="modal-title">
										Agregar Certificación de Producto
									</h5>
									<button
										type="button"
										class="close"
									...
									<div class="form-group">
										...
										<div class="custom-file">
											...
											<label
												...
												for="addOfferCertFile"
												><i class="fas fa-file-pdf"></i>
												{{ upload_file_name }}}</label
											>
											<small
												><span class="text-muted"
								...
								<div class="modal-footer">
									<button
										type="button"
										class="btn btn-add"
										@click.prevent.stop="addOfferCert()"
									>
										Agregar
									</button>
									<button
...
export default {
	...
	methods: {
		...
		addOfferCert() {
			Swal.fire({
				...
				if (res.value == true) {
					...
					axios
						.post("/profile/add-product-cert", formData, config)
						.then(res => {
		...
		deleteOfferCert(certification_id) {
			Swal.fire({
				title:
					"¿Estas seguro que deseas eliminar el certificado de este producto?",
				...
				if (res.value == true) {
					axios
						.delete(
							`/profile/delete-product-cert/${certification_id}`
						)
						.then(res => {
							console.log(res.data);
							Swal.fire({
