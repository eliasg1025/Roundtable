<template>
	<div
		:id="'certxadsad' + certification.id"
		class="modal fade"
		tabindex="-1"
		role="dialog"
		aria-hidden="true"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div>
						...
						<div class="custom-file">
							...
							/>
							<label class="custom-file-label" for="editCertFile"
								><i class="fas fa-file-pdf"></i>
								{{ upload_offer_cert_name }}</label
							>
							<small
								><span class="text-muted"
									>Solamente se aceptará archivos de formato
									pdf</span
								></small
							>
						</div>
				...
				<div class="modal-footer">
					<button
						...
						@click.prevent.stop="editOfferCert()"
					>
						Guardar cambios
					</button>
					<button
...
export default {
	...
	methods: {
		...
		editOfferCert() {
			Swal.fire({
				title:
					"¿Realmente deseas editar los datos de este certificado?",
				type: "warning",
				...
				if (res.value == true) {
					...
					axios
						.post(
							`/profile/edit-product-cert/${this.certification.id}`,
							formData,
							config
						)
						.then(res => {
